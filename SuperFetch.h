...
namespace spf
{
	...
	public:
		...
		result(E error) : m_error(error), m_has_value(false) {}

		bool has_value() const noexcept { return m_has_value; }
		explicit operator bool() const noexcept { return m_has_value; }

		T& value()& { if (!m_has_value) throw std::runtime_error("bad result access"); return m_value; }
		const T& value() const& { if (!m_has_value) throw std::runtime_error("bad result access"); return m_value; }

		T&& value()&& { if (!m_has_value) throw std::runtime_error("bad result access"); return std::move(m_value); }
	...
	public:
		...
		static result<memory_map> current();
	...
	inline result<memory_map> memory_map::current()
	{
		if (!raise_privilege())
		{
			return spf_error::raise_privilege;
		}
		...
		if (mm.ranges_.empty())
			return spf_error::query_ranges;

		for (auto const& [base_pfn, page_count] : mm.ranges_)
		{
			auto const buffer_length = static_cast<ULONG>(sizeof(PF_PFN_PRIO_REQUEST) +
				sizeof(MMPFN_IDENTITY) * page_count);
